...
Mesh generateTextGeometryBuffer(std::string text, float characterHeightOverWidth, float totalTextWidth) {
    ...
    for(unsigned int i = 0; i < text.length(); i++)
    {
        ...
        float baseXText = stride * float(text.at(i));
        ...
        mesh.textureCoordinates.at(4 * i + 0) = {baseXText, 0};
        mesh.textureCoordinates.at(4 * i + 1) = {baseXText + stride, 0};
        mesh.textureCoordinates.at(4 * i + 2) = {baseXText + stride, 1};
        mesh.textureCoordinates.at(4 * i + 3) = {baseXText, 1};
    }
